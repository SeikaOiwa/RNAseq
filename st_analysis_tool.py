...
def read_trime():
    ...
    st.write('「Biotools」にreadデータ(fasatq.gz)を保存してください') 
...
def multi_mapping():
    ...
    st.markdown("""- Biotoolsに「ゲノムデータ(*.fasta)を保管してください」""")
    st.markdown("""- Biotools/Mapping下にRNAseqデータ毎にフォルダを作成し、「RNAseqデータ(*.fastq.gz)を保管してください」""")
...
def p_mapping():
    ...
    st.subheader('1) データの選択')
    st.markdown("""- Biotoolsに「ゲノムデータ(*.fasta)とRNAseqデータ(*.fastq.gz)を保管してください」""")
    # genome_data
    genome_data_lists_ = glob.glob(f'{base_path}/*.fasta')
    genome_data_lists = [os.path.basename(i) for i in genome_data_lists_]
    genome_data = st.selectbox("ゲノムデータを選択",genome_data_lists,placeholder="＊.fastaを1種類選択")
...
def p_count():
    ...
    st.subheader('1) 準備')
    st.markdown("""- BiotoolsにMappingデータ（*.bam)とアノテーションデータ(*.gtf)を保管してください」""")
    # mapping_data
